...
export function calculateHeadMinimumThickness(data: HeadData): number {
  ...
  switch (headType) {
    ...
    case 'torispherical':
      ...
      // Standard ASME F&D head: L (crown radius) ≈ Outside Diameter (Do)
      // We approximate Do as D + 0.5" (where D = 2R) for conservative estimate
      const L_approx = (2 * R) + 0.5;
      t = (0.885 * P * L_approx) / (S * E - 0.1 * P);
      break;
