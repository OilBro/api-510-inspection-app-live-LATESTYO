...
async function createInspection() {
  ...
  const now = new Date();
  const inspectionDate = new Date('2017-06-20');
  ...
  await db.insert(schema.inspections).values({
    ...
    vesselName: 'Triethylamine Storage Tank',
    manufacturer: 'Riley Beard Inc.',
    serialNumber: '1408365-01-A',
    yearBuilt: 1977,
    inspectionDate: inspectionDate,
    designPressure: "280",
    designTemperature: "125",
    operatingPressure: "170",
    materialSpec: 'SA-612',
    allowableStress: "20000",
    jointEfficiency: "1",
    radiographyType: 'RT-1',
  ...
  await db.insert(schema.professionalReports).values({
    id: reportId,
    inspectionId: inspectionId,
    reportNumber: '54-11-001-2017',
    clientName: 'Valero',
  ...
  for (const comp of components) {
    await db.insert(schema.componentCalculations).values({
      id: comp.id,
      reportId: reportId,
      componentName: comp.componentName,
      componentType: comp.componentType,
      materialCode: comp.materialCode,
      designTemperature: comp.designTemperature,
      designMAWP: comp.designMAWP,
      insideDiameter: comp.insideDiameter,
      nominalThickness: comp.nominalThickness,
      previousThickness: comp.previousThickness,
      actualThickness: comp.actualThickness,
      minRequiredThickness: comp.minRequiredThickness,
      corrosionRate: comp.corrosionRate,
      remainingLife: comp.remainingLife,
      allowableStress: comp.allowableStress,
      jointEfficiency: comp.jointEfficiency,
      createdAt: now,
      updatedAt: now
    });
  ...
  for (const tml of tmlReadings) {
    await db.insert(schema.tmlReadings).values({
      id: nanoid(),
      inspectionId: inspectionId,
      cmlNumber: tml.cml,
      componentType: tml.component,
  ...
  for (const noz of nozzles) {
    await db.insert(schema.nozzleEvaluations).values({
      id: noz.id,
      inspectionId: inspectionId,
      nozzleNumber: noz.nozzleId,
      nozzleDescription: noz.service,
      nominalSize: noz.size,
      pipeNominalThickness: noz.nominal,
      actualThickness: noz.actual,
      createdAt: now,
      updatedAt: now
    });
  }
